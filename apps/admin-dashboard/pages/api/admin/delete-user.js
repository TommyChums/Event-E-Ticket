...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data: { user }, error } = await auththenticatedSupabase.auth.admin.deleteUser(user_id);

    if (error) {
      return res.status(500).json({ error: 'Error deleting the user' });
    }

    return res.status(200).json({ message: 'Successfully deleted the user', user });
  } else {
